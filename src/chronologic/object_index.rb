module Chronologic
  class ObjectIndex < BaseIndex
    ...
    def load(events = [])
      index = lucene.index.IndexWriter.new(
        directory, analyzer, !exists?, max_field_length)
      ...
      events.each do |event|
        ...
        event.each do |fname, value|
          if fname == :id
            field = lucene.document.Field.new(
              fname.to_s, value.to_s, stored, not_analyzed)
          else
            field = lucene.document.Field.new(
              fname.to_s, value.to_s, not_stored, not_analyzed)
          end
          document.add(field)
        end
    ...
    def sample(query_string, options = {:size => 10})
      ...
      if query_string.empty?
        ...
        parser = lucene.queryParser.QueryParser.new(
          lucene_version, "created_at", analyzer);
        query = parser.parse(query_string);
      end
      
      sort = lucene.search.Sort.new(random_sort_field)
      collector = lucene.search.TopFieldCollector.create(
        sort, options[:size], true, false, false, false);
